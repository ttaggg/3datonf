...
class GaussianFourierFeatureTransform(nn.Module):
    ...
    def __init__(self, network_spec, in_channels, mapping_size=256, scale=10):
        ...
        self.scale = scale
        self.register_buffer("_B",
                             torch.randn((in_channels, mapping_size)) * scale)
...
class IOSinusoidalEncoding(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        ...
        if self.enc_layers:
            layer_enc = fourier_encode(layernum, self.max_freq,
                                       self.num_bands)  # (L, 2 * num_bands + 1)
        else:
            layer_enc = torch.zeros((L, 2 * self.num_bands + 1),
                                    device=device,
                                    dtype=dtype)
        inpnum = torch.linspace(-1.,
                                1.,
                                steps=self.n_in,
                                device=device,
                                dtype=dtype)
        inp_enc = fourier_encode(inpnum, self.max_freq,
                                 self.num_bands)  # (n_in, 2 * num_bands + 1)
        outnum = torch.linspace(-1.,
                                1.,
                                steps=self.n_out,
                                device=device,
                                dtype=dtype)
        out_enc = fourier_encode(outnum, self.max_freq,
                                 self.num_bands)  # (n_out, 2 * num_bands + 1)
        ...
        for i in range(L):
            ...
            bias_enc_i = layer_enc[i][None, :, None].expand(
                b, d, bias.shape[-1])  # (B, d, n_row)
            if i == 0:
                # weight has shape (B, c_in, n_out, n_in)
                inp_enc_i = inp_enc.transpose(0, 1).unsqueeze(0).unsqueeze(
                    -2)  # (1, d, 1, n_col)
                for _ in axes[2:]:
                    inp_enc_i = inp_enc_i.unsqueeze(-1)
                enc_i = enc_i + inp_enc_i
            if i == len(wsfeat) - 1:
                out_enc_i = out_enc.transpose(0, 1).unsqueeze(0).unsqueeze(
                    -1)  # (1, d, n_row, 1)
                for _ in axes[2:]:
...
class LearnedPosEmbedding(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        ...
        for i in range(len(self.network_spec)):
            ...
            if i == 0:
                weight = weight + self.inp_weight_arrange(
                    self.inp_emb.weight)[(Ellipsis, *filter_dims)]
            if i == len(wsfeat.weights) - 1:
                weight = weight + self.out_weight_arrange(
                    self.out_emb.weight)[(Ellipsis, *filter_dims)]
