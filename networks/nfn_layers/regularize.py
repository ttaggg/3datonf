...
class ParamLayerNorm(nn.Module):
    ...
    def __init__(self, network_spec: NetworkSpec, channels):
        ...
        for i in range(len(network_spec)):
            if i == 0:
                ...
                w_shape = (channels,)
                v_shape = (channels,)
            self.add_module(f"norm{i}_w",
                            nn.LayerNorm(normalized_shape=w_shape))
            self.add_module(f"norm{i}_v",
                            nn.LayerNorm(normalized_shape=v_shape))
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        ...
        for i, (weight, bias) in enumerate(wsfeat):
            ...
            if i == 0:
                out_weights.append(
                    w_norm(weight.transpose(-3, -2)).transpose(-3, -2))
                out_biases.append(
                    v_norm(bias.transpose(-1, -2)).transpose(-1, -2))
            elif i == len(wsfeat) - 1:
                out_weights.append(
                    w_norm(weight.transpose(-3, -1)).transpose(-3, -1))
