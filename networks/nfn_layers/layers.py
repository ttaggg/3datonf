...
class NPLinear(nn.Module):
    ...
    def __init__(self,
        ...
        if io_embed:
            ...
            self.out_bias_embed = nn.Parameter(
                torch.randn(1, in_channels, n_out))

    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        wsfeat = shape_wsfeat_symmetry(wsfeat, self.network_spec)
        if self.io_embed:
            new_weights = (wsfeat.weights[0] + self.in_embed,
                           *wsfeat.weights[1:-1],
                           wsfeat.weights[-1] + self.out_embed)
            new_biases = (*wsfeat.biases[:-1],
                          wsfeat.biases[-1] + self.out_bias_embed)
        ...
        wb_means = torch.cat(rowcol_means + bias_means,
                             dim=-1)  # (B, 2 * C_in * n_layers)
        out_weights, out_biases = [], []
        for i in range(len(self.network_spec)):
            weight, bias = wsfeat[i]
            ...
            if i > 0:
                row_bdcst.extend([col_means[i - 1],
                                  biases[i - 1]])  # (B, C_in, ncol)
            if i < len(wsfeat.weights) - 1:
                col_bdcst.append(row_means[i + 1])  # (B, C_in, nrow)
            col_bdcst = torch.cat(col_bdcst, dim=-2)
            z3 = getattr(self, f"layer_{i}_r")(torch.cat(
                row_bdcst, dim=-2)).unsqueeze(-2)  # (B, C_out, 1, ncol)
            ...
            u2 = getattr(self, f"bias_{i}_rc")(wb_means).unsqueeze(
                -1)  # (B, C_out, 1)
...
class HNPLinearAngle(nn.Module):
        ...
        for i in range(self.L):
            ...
            if i == 0:
                ...
                self._angle_emb[i] = nn.Sequential(
                    ...
                    nn.Linear(64, n_in * fac_i * out_channels))
                # torch.nn.init.zeros_(self._angle_emb[i][0].weight)
                # torch.nn.init.zeros_(self._angle_emb[i][0].bias)
                # torch.nn.init.zeros_(self._angle_emb[i][2].weight)
                # torch.nn.init.zeros_(self._angle_emb[i][2].bias)
                ...
                self._angle_emb_bias[i] = nn.Sequential(
                    nn.Linear(2, 64), nn.ReLU(), nn.Linear(64, out_channels))
                # torch.nn.init.zeros_(self._angle_emb_bias[i][0].weight)
                # torch.nn.init.zeros_(self._angle_emb_bias[i][0].bias)
                # torch.nn.init.zeros_(self._angle_emb_bias[i][2].weight)
                # torch.nn.init.zeros_(self._angle_emb_bias[i][2].bias)
                ...
                self._angle_emb[i] = nn.Sequential(
                    nn.Linear(2, 64), nn.ReLU(),
                    nn.Linear(64, n_out * fac_i * out_channels))
                # torch.nn.init.zeros_(self._angle_emb[i][0].weight)
                # torch.nn.init.zeros_(self._angle_emb[i][0].bias)
                # torch.nn.init.zeros_(self._angle_emb[i][2].weight)
                # torch.nn.init.zeros_(self._angle_emb[i][2].bias)
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        ...
        for i in range(self.L):
            ...
            if i == 0:
                ...
                z5 = torch.reshape(z5, (z.shape[0], 128, 32, 1))
                # z = torch.cat([z, z5], dim=-1)
                # z = self._angle_emb_merge[i](z)
                z = z + z5
...
def simple_attention(q, k, v, dropout=None):
    ...
    attn = torch.softmax(q @ k.transpose(-2, -1) / math.sqrt(k.shape[-1]),
                         dim=-1)
...
class NPAttention(nn.Module):
    ...
    def __init__(
        ...
        if share_projections:
            ...
            self.weight_to_qkv = nn.ModuleList(
            )  # maps channel dim k * k * c --> c
        ...
        self.split_heads = Rearrange("b (nh ch) ... -> b nh ch ...",
                                     nh=self.nh,
                                     ch=3 * self.ch)
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        ...
        for i in range(-1, len(wsfeat)):
            inp = []
            if i > -1:
                n_i, n_im1 = wsfeat.weights[i].shape[-2], wsfeat.weights[
                    i].shape[-1]
            ...
            q_i, k_i, v_i = inp.tensor_split(
                3, dim=-2)  # (B, nh, n_im1 + 1 + n_ip1, c, n_i)
            ...
            if i > -1:
                ...
                out_weights_i = torch.flatten(out[:, :, :n_im1].permute(
                    0, 1, 3, 4, 2),
                                              start_dim=1,
                                              end_dim=2)
            ...
            if i < len(wsfeat) - 1:
                ...
                out_weights_ip1 = torch.flatten(out[:, :, idx:].transpose(2, 3),
                                                start_dim=1,
                                                end_dim=2)
                if not self.share_projections:
