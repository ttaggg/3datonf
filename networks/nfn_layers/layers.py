...
class HNPLinearAngleMerge(nn.Module):
    ...
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels, input_dim, hidden_size):
        super().__init__()
        self.network_spec = network_spec
        n_in, n_out = network_spec.get_io()
        self.n_in, self.n_out = n_in, n_out
        ...
        for i in range(self.L):
            ...
            if i == 0:
                ...
                self._angle_emb_bias[i] = nn.Sequential(
                    nn.Linear(input_dim, hidden_size), nn.ReLU(), nn.Linear(hidden_size, out_channels))
                ...
                self._angle_emb[i] = nn.Sequential(
                    nn.Linear(input_dim, hidden_size), nn.ReLU(),
                    nn.Linear(hidden_size, 32 * n_out * fac_i * out_channels))
                self.add_module(f"layer_{i}_angle", self._angle_emb[i])
    ...
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels, input_dim, hidden_size):
        ...
        for i in range(self.L):
            ...
            if i == 0:
                ...
                # BIASES
                self._angle_emb_bias[i] = nn.Sequential(
                    nn.Linear(input_dim, 64), nn.ReLU(), nn.Linear(64, out_channels))
                self.add_module(f"layer_{i}_angle_bias",
                                self._angle_emb_bias[i])
