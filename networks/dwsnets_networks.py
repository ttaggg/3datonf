...
from networks.dwsnets_layers import BN, DownSampleDWSLayer, Dropout, DWSLayer, InvariantLayer, ReLU
...
class MLPModel(nn.Module):
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        ...
        weight = torch.cat([w.flatten(start_dim=1) for w in all_weights],
                           dim=-1)
        ...
        for s in weight_shape:
            weight.append(weights[:,
                                  w_index:w_index + s.numel()].reshape(bs, *s))
            w_index += s.numel()
        w_index = 0
        for s in bias_shape:
...
class DWSModel(nn.Module):
    ...
    def __init__(
        ...
        assert len(
            weight_shapes
        ) > 2, "the current implementation only support input networks with M>2 layers."
        ...
        if self.add_skip:
            ...
            with torch.no_grad():
                torch.nn.init.constant_(self.skip.weight,
                                        1. / self.skip.weight.numel())
        ...
        if input_dim_downsample is None:
            ...
            for i in range(n_hidden):
                if bn:
                    layers.append(
                        BN(hidden_dim, len(weight_shapes), len(bias_shapes)))
                ...
                layers.extend([
                    ...
                    DWSLayer(
                        ...
                        out_features=hidden_dim
                        if i != (n_hidden - 1) else output_features,
                        reduction=reduction,
                        bias=bias,
                        n_fc_layers=n_fc_layers,
...
class DWSModelForClassification(nn.Module):
    ...
    def __init__(
        ...
        self.clf = InvariantLayer(
            ...
            bias_shapes=bias_shapes,
            in_features=hidden_dim
            if equiv_out_features is None else equiv_out_features,
    ...
    def forward(self,
                x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]],
                return_equiv=False):
        x = self.layers(x)
