...
class GeneralMatrixSetLayer(BaseLayer):
    ...
    def forward(self, x):
        if self.in_index == self.out_index:
        ...
        elif (self.in_index == self.out_index -
              1):  # self.in_shape[-1] == self.out_shape[0]:
            # i -> j  where i=j-1
            if self.first_dim_is_input:
                # i=0 and j=1
                # (bs, d1, d0 * in_features)
            ...
            if self.last_dim_is_input:
                ...
                x = self.set_layer(x)
                # (bs, d1, d0, out_features)
                x = x.reshape(x.shape[0], x.shape[1], self.out_shape[0],
                              self.out_features)
...
class SetKroneckerSetLayer(BaseLayer):
    ...
    def forward(self, x):
        ...
        pooled_rows = self._reduction(
            x, dim=1, keepdim=True)  # [b, d1, d2, f] -> [b, 1, d2, f]
        out_rows = self.lin_n(pooled_rows)  # [b, 1, d2, f] -> [b, 1, d2, f']
        # cols
        pooled_cols = self._reduction(
            x, dim=2, keepdim=True)  # [b, d1, d2, f] -> [b, d1, 1, f]
        ...
        new_features = (out_all + out_rows + out_cols +
                        out_both) / 4.0  # [b, d1, d2, f']
        return new_features
...
class FromFirstLayer(BaseLayer):
    ...
    def __init__(
        ...
        if self.last_dim_is_output:
            # i=0, j=L-1
            in_features = self.in_features * self.in_shape[
                0]  # d0 * in_features
            out_features = self.out_features * self.out_shape[
                1]  # dL * out_features
            self.layer = self._get_mlp(in_features=in_features,
                                       out_features=out_features,
                                       bias=bias)
    ...
    def forward(self, x):
        if self.last_dim_is_output:
            ...
            x = (x.reshape(x.shape[0], self.out_shape[-1],
                           self.out_features).unsqueeze(1).repeat(
                               1, self.out_shape[0], 1, 1))
        else:
            # i=0, j != L-1
            # (bs, d0, d1, in_features)
            # (bs, d0, in_features)
            ...
            for j in range(self.n_layers):
                ...
                if i == j:
                    ...
                    self.layers[f"{i}_{j}"] = FromFirstLayer(
                        ...
                        last_dim_is_output=(
                            j == self.n_layers -
                            1),  # todo: make sure this condition is correct
                    )
                elif j == 0:
...
if __name__ == "__main__":
    ...
    print(len(matrices))
    weight_block = WeightToWeightBlock(in_features=12,
                                       out_features=24,
                                       shapes=tuple(
                                           m.shape[1:3] for m in matrices))
    ...
    assert torch.allclose(out[0][:, :, perm1, :],
                          out_perm[0],
                          atol=1e-5,
                          rtol=0)
    assert torch.allclose(out[1][:, perm1, :, :][:, :, perm2, :],
                          out_perm[1],
                          atol=1e-5,
                          rtol=0)
    assert torch.allclose(out[2][:, perm2, :, :][:, :, perm3, :],
                          out_perm[2],
                          atol=1e-5,
                          rtol=0)
    assert torch.allclose(out[3][:, perm3, :, :][:, :, perm4, :],
                          out_perm[3],
                          atol=1e-5,
                          rtol=0)
    assert torch.allclose(out[4][:, perm4, :, :],
                          out_perm[4],
                          atol=1e-5,
                          rtol=0)
