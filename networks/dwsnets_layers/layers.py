...
from networks.dwsnets_layers.base import BaseLayer, GeneralSetLayer
from networks.dwsnets_layers.bias_to_bias import BiasToBiasBlock
from networks.dwsnets_layers.bias_to_weight import BiasToWeightBlock
from networks.dwsnets_layers.weight_to_bias import WeightToBiasBlock
from networks.dwsnets_layers.weight_to_weight import WeightToWeightBlock
...
class BN(nn.Module):
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        ...
        for i, (m, w) in enumerate(zip(self.weights_bn, weights)):
            shapes = w.shape
            new_weights[i] = (m(w.permute(0, 3, 1,
                                          2).flatten(start_dim=2)).permute(
                                              0, 2, 1).reshape(shapes))
...
class ReLU(nn.Module):
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        weights, biases = x
        return tuple(F.relu(t) for t in weights), tuple(
            F.relu(t) for t in biases)
...
class LeakyReLU(nn.Module):
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        weights, biases = x
        return tuple(F.leaky_relu(t) for t in weights), tuple(
            F.relu(t) for t in biases)
...
class DWSLayer(BaseLayer):
    ...
    def _apply_off_diag_penalty(name):
        if "weight_to_weight" in name or "bias_to_bias" in name:
            ...
            return (len(set(name.split(".")[2].split("_")))
                    == 2) or ("skip" not in name)
        else:
            return True
    ...
    def _init_model_params(self, scale, off_diag_penalty=1.0):
        for n, m in self.named_modules():
            if isinstance(m, nn.Linear):
                ...
                off_diag_penalty_ = (off_diag_penalty if
                                     self._apply_off_diag_penalty(n) else 1.0)
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        ...
        new_weights = tuple((w0 + w1) / self.n_matrices for w0, w1 in zip(
            new_weights_from_weights, new_weights_from_biases))
        new_biases = tuple(
        ...
        if self.add_skip:
            ...
            new_weights = tuple(
                ws + w for w, ws in zip(new_weights, skip_out[0]))
...
class InvariantLayer(BaseLayer):
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        ...
        pooled_weights = torch.cat(
            [
                self._reduction(w.permute(0, 3, 1, 2).flatten(start_dim=2),
        ...
        pooled_biases = torch.cat(
            [self._reduction(b, dim=1) for b in biases[:-1]],
            dim=-1)  # (bs, (len(biases) - 1) * in_features)
...
class NaiveInvariantLayer(BaseLayer):
    ...
    def __init__(
        ...
        self.bias_shapes = bias_shapes
        n_layers = len(weight_shapes) + len(bias_shapes)
        self.layer = self._get_mlp(in_features=in_features * n_layers,
                                   out_features=out_features,
                                   bias=bias)
    ...
    def forward(self, x: Tuple[Tuple[torch.tensor], Tuple[torch.tensor]]):
        ...
        pooled_weights = torch.cat(
            [
                self._reduction(w.permute(0, 3, 1, 2).flatten(start_dim=2),
                                dim=2) for w in weights
            ],
            dim=-1,
        )  # (bs, len(weights) * in_features)
        pooled_biases = torch.cat([self._reduction(b, dim=1) for b in biases],
                                  dim=-1)  # (bs, len(biases) * in_features)
        pooled_all = torch.cat([pooled_weights, pooled_biases],
                               dim=-1)  # (bs, num layers * in_features)
        return self.layer(pooled_all)
